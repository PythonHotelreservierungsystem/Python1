...
### Instruktion zur Ausführung der User Stories
Für die User Stories haben wir ein separates Notebook user_stories.ipynb erstellt. Bitte führen Sie die Zellen in diesem Notebook von oben nach unten der Reihe nach aus, um sicherzustellen, dass alle Abhängigkeiten und Variablen korrekt geladen werden. Wichtig: Für die Ausführung der Admin-bezogenen User Stories müssen Sie als Administrator angemeldet sein. Dies wird in Zelle 8 des Notebooks umgesetzt und sichergestellt. Eine detaillierte Beschreibung zu den einzelnen User Stories sowie Informationen, wie wir diese umgesetzt haben, finden Sie ebenfalls im README.md weiter unten.
...
Der Gast gibt den gewünschten Ort und Zeitraum des Aufenthalts an: city = "Genève", check_in = date(2025, 8, 11), check_out = date(2025, 8, 17). Diese Daten bilden die Grundlage für die Verfügbarkeitsprüfung. Die Methode find_available_hotels_by_city_and_dates() kombiniert zwei Filter: Den Stadtfilter (city) und die Zimmerverfügbarkeit zwischen check_in und check_out. Dabei wird geprüft, ob in einem Hotel mindestens ein Zimmer in diesem Zeitraum frei ist: verfuegbare_hotels = manager.find_available_hotels_by_city_and_dates(city, check_in, check_out, booking_dao). Diese Umsetzung stellt sicher, dass Gäste ausschließlich verfügbare Unterkünfte sehen – ein essenzieller Bestandteil jeder Hotelreservierungsplattform. 
...
Zu Beginn legt der Nutzer die Stadt, den gewünschten Reisezeitraum, die benötigte Zimmerkapazität sowie die gewünschte Mindestanzahl an Sternen fest: stadt = "Genève", check_in = datetime(2025, 8, 23), check_out = datetime(2025, 8, 24), min_sterne = 4. gaesteanzahl = 2. Die Methode find_hotels_by_criteria() nimmt alle Kriterien auf einmal entgegen und führt eine kombinierte Filterung durch. Sie gibt nur die Hotels zurück: die sich in der gewünschten Stadt befinden, mindestens ein Zimmer für die gewünschte Gästezahl anbieten, über mindestens die angegebene Anzahl Sterne verfügen und im gewünschten Zeitraum über ein freies Zimmer verfügen:  verfuegbare_hotels = manager.find_hotels_by_criteria(city=stadt, check_in_date=check_in, check_out_date=check_out, min_stars=min_sterne, guest_count=gaesteanzahl, booking_da=booking_dao). Wurden passende Hotels gefunden, erfolgt eine strukturierte Ausgabe mit Name, Straße, PLZ und Stadt: print(f"{hotel.name} – {hotel.address.street}, {hotel.address.zip_code} {hotel.address.city}"). Falls kein Hotel alle Kriterien erfüllt, wird dies dem Nutzer klar und verständlich mitgeteilt: print("Keine passenden Hotels für die angegebenen Kriterien gefunden."). Der Code stellt sicher, dass Gäste gezielt und bequem nach exakt den Hotels suchen können, die all ihren Anforderungen entsprechen – eine zentrale Voraussetzung für ein positives Buchungserlebnis.

### User Story 1.6; Ich möchte die folgenden Information pro Hotel sehen: Name, Adresse, Anzahl der Sterne.
Die Methode show_all_hotel_infos() gibt eine Liste von Hotel-Einträgen zurück. Jeder Eintrag enthält alle erforderlichen Felder (z. B. Name, Straße, Stadt, PLZ, Sterne). Die Hotelinformationen abrufen: infos = manager.show_all_hotel_infos(). Die Liste wird übersichtlich mit Aufzählungszeichen (-) Zeile für Zeile ausgegeben. Das Format der einzelnen eintrag-Strings wird bereits in der Methode definiert: print("Verfügbare Hotels:"), for eintrag in infos: print(" -", eintrag). Damit haben Gäste eine einfache Möglichkeit, sich einen Überblick über alle verfügbaren Hotels samt ihrer zentralen Eigenschaften zu verschaffen – ein essenzieller erster Schritt im Buchungsprozess.
